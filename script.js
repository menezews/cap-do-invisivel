...
// sobrepõem no mapa — número maior fica por CIMA. Se criar uma
// categoria nova e não colocar ela aqui, ela fica no meio
// (camada 2) por padrão.
  ...
  locais.forEach((local, indice) => {
    ...
    if (temImagens(local)) {
      ...
      popup.on('open', () => {
        ...
        // Registra esse popup como o "atual" pro teclado — só
        // faz diferença se tiver mais de uma foto (senão não
        // tem seta nenhuma pra função mexer mesmo).
        if (local.imagens.length > 1) {
          ...
          popup.once('close', () => {
            // só limpa se ainda for o mesmo popup (evita que o
            // fechamento de um popup antigo apague o controlador
            // de um popup mais novo que já abriu depois)
